...
description: Learn how to connect RankRocket ContentIQ with third-party tools and use webhooks for seamless data flow in your workflow.
---

## Overview

RankRocket ContentIQ integrates seamlessly with popular third-party tools to enhance your Facebook analytics workflow. Connect your Facebook pages directly, set up webhooks for real-time notifications, export data to Google Sheets or BI platforms, and build custom solutions via API.
...
Connect RankRocket ContentIQ to the Facebook Graph API to pull page and post insights automatically.
...
<Steps>
  ...
  <Step title="Add to RankRocket ContentIQ" icon="link">
    In your RankRocket ContentIQ dashboard at `https://fbaudienceanalyzer.abacusai.app`, navigate to Settings &gt; Integrations. Paste the token and select your pages.
  </Step>
</Steps>
...
Configure webhooks in your dashboard under Integrations &gt; Webhooks. Provide your endpoint URL, such as `https://your-webhook-url.com/rankrocket-contentiq`.
...
<Tabs>
  <Tab title="Zapier" icon="zap">
    <Steps>
      <Step title="Trigger setup">
        Select RankRocket ContentIQ as trigger app. Choose `New Engagement Alert`.
      </Step>
      <Step title="Action">
  ...
  <Tab title="Make.com" icon="play">
    Create a scenario with RankRocket ContentIQ webhook module. Parse JSON payload and route to Google Sheets or HubSpot.
  </Tab>
</Tabs>
...
<CodeGroup tabs="Zapier,Direct API">
  ```javascript
  // Google Apps Script to fetch from RankRocket ContentIQ
  function importRankRocketContentIQData() {
  ...
  sheet = gc.open("RankRocket ContentIQ Insights").sheet1
